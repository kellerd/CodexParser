...
type Result<'a,'b> =
    | Success of 'a
    | Failure of ParserLabel * ParserError * ('b option)

/// A Parser structure has a parsing function & label
type Parser<'a,'b> = {
    parseFn : (Input -> Result<'a * Input,'b * Input>)
    label:  ParserLabel 
    }
...
let bindP<'a,'b> (f:'a->Parser<'b>) (p:Parser<'a>) : Parser<'b> =
    let label = "unknown"
    let innerFn input =
        let result1 = run p input 
        match result1 with
        | Failure (label,err,Some (value1,remainingInput)) -> 
             Failure (label,err,Some (value1,remainingInput))
        | Success (value1,remainingInput) ->
            // apply f to get a new parser
            let p2 = f value1
            // run parser with remaining input
            let result = run p2 remainingInput
            result
